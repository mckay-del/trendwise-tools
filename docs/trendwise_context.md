...
- **Implemented as a confirmed GHL workflow** (all mechanisms verified in-account): signup product purchased (one-time = setup + first month) → Wait 31 days (burn prepaid month) → If/Else on "Current Day of month" (≤14 → wait for the 15th; else → wait for the 1st, via "Until a recurring window opens" monthly waits) → "Send recurring invoice" (Start = Action Date, End = Never, Monthly, Autopayment ON using the signing card). Two products per tier: a one-time signup product (setup + first month) and a monthly recurring product. Full detail in the standalone `Anchor_Billing_Workflow_Build.md`.
